...
| A06 | Migration of existing employee records | Partial | Database-backed employee records exist; no complete repeatable STRH migration package or validation report was found. | Prepare mapping, import validation, duplicate handling, rejected-row report, and signed migration validation. |
...
| C10 | PSIPOP-patterned Plantilla database | Implemented | Dedicated Plantilla items, occupancy, vacancy, salary/funding, effective dates, history, API, and UI are implemented with active-occupancy constraints. | Encode and validate the STRH-approved PSIPOP source data before UAT. |
...
| D13 | Payroll deduction support for LWOP, tardiness, and undertime | Partial | DTR stores tardiness and undertime and leave supports LWOP; no payroll integration or approved deduction export was found. | Define payroll interface, cutoff/locking, correction handling, and cross-checking with STRH. |
