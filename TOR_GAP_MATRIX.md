...
| C09 | Maintain employee job-related history | Partial | Salary and service rows exist, but there is no governed personnel-action workflow connecting old/new position, item, assignment, and salary. | Implement approved personnel movements that update the current profile and preserve immutable history. |
| C10 | PSIPOP-patterned Plantilla database | Not implemented | No dedicated plantilla-item/occupancy schema or UI was found. | Build authorized positions, item numbers, funding, salary data, occupancy, vacancy, and effective-dated history. |
...
| C12 | Appointment, assignment, designation, separation, qualification standard, and salary-action records | Partial | Service and salary rows can store some information; formal action types, approvals, attachments, and effectivity are absent. | Implement structured personnel-action records and approval lifecycle. |
| C13 | Original appointment, promotion, transfer, renewal, reassignment, detail, job rotation, reclassification, step increment, resignation, retirement, termination, death, etc. | Not implemented | No general personnel-movement module was found. | Implement TOR action types with old/new snapshots, authority, effectivity, attachment, approval, posting, and reversal. |
